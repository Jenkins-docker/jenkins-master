...
RUN set -x \
    ...
    && apt-get install -y vim wget subversion \
...
RUN useradd -d "$JENKINS_HOME" -u ${uid} -m -s /bin/bash ${user}\
...
RUN curl -fL https://github.com/krallin/tini/releases/download/v${TINI_VERSION}/tini-static -o /bin/tini \
    && chmod +x /bin/tini \
